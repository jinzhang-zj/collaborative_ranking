...
// Date: 11/26/2014
//
// The script is the sequential version which use liblinear training function
//
// Compile: g++ -std=C++11 -O3 -g collaborative_ranking_seq.cpp
...
void Problem::alt_rankSVM () {
	...
	for (int i = 0; i < this->n_train_comps; ++i) {
		...
		for (int j = 0; j < this->rank; ++j) {
			B[i][j].index = this->g.ucmp[i].item1_id * rank + j + 1;
			B[i][j + rank].index = this->g.ucmp[i].item2_id * rank + j + 1;
		}
	...
	for (int iter = 0; iter < 5; ++iter) {
		...
		for (int i = 0; i < this->n_train_comps; ++i) {
			...
			for (int j = 0; j < this->rank; ++j) {
				B[i][j].value = U[this->g.ucmp[i].user_id * this->rank + j];		// U_i
				B[i][j + rank].value = -U[this->g.ucmp[i].user_id * this->rank + j];	// -U_i
			}		
		}
